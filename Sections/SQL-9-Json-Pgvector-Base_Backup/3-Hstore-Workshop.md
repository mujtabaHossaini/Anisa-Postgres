...
```
SELECT attr FROM books;Code language: SQL (Structured Query Language) (sql)
```
...
```
UPDATE books
SET attr = attr || '"freeshipping"=>"yes"' :: hstore;Code language: SQL (Structured Query Language) (sql)
```
...
```
UPDATE books
SET attr = attr || '"freeshipping"=>"no"' :: hstore;Code language: SQL (Structured Query Language) (sql)
```
...
FROM
	books
WHERE
	attr ? 'publisher';Code language: SQL (Structured Query Language) (sql)
```
...
FROM
	books
WHERE
	attr @> '"weight"=>"11.2 ounces"' :: hstore;Code language: SQL (Structured Query Language) (sql)
```
...
FROM
	books
WHERE
	attr ?& ARRAY [ 'language', 'weight' ];Code language: SQL (Structured Query Language) (sql)
```
